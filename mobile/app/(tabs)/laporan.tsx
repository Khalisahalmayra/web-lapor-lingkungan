...
export default function LaporScreen() {
  ...
  try {
    ...
    // ✅ Sukses — tampilkan modal lalu otomatis redirect ke beranda
    setShowSuccess(true);
    setTimeout(() => {
      setShowSuccess(false);
      resetForm();
      router.replace("/(tabs)/beranda");
    }, 2000); // auto redirect setelah 2 detik
    ...
    <KeyboardAvoidingView style={s.root} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      ...
      <SuccessModal
        ...
        onClose={() => {
            setShowSuccess(false);
            resetForm();
            router.replace("/(tabs)/beranda");   // ← Kembali ke Beranda
        }}
        onNew={() => {
            setShowSuccess(false);
            resetForm();
            router.replace("/(tabs)/laporan");   // ← Buat Laporan Baru
        }}
        />
    </KeyboardAvoidingView>
  );
